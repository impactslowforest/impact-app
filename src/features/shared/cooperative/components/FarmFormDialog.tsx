  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{isEditing ? t('edit_farm') : t('add_farm')}</DialogTitle>
          <DialogDescription>
            {isEditing ? `${editingItem?.farm_code} — ${editingItem?.farm_name}` : t('fill_all_fields', 'Fill in all required fields')}
          </DialogDescription>
        </DialogHeader>
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
            <div className="bg-primary-700 px-4 py-2">
              <h3 className="text-[12px] font-bold text-white uppercase tracking-wider">{t('farmer')}</h3>
            </div>
            <div className="flex items-center gap-4 px-4 py-3 bg-white">
              <span className="text-[13px] font-medium text-primary-700 w-2/5 shrink-0">{t('farmer')} *</span>
              <select
                name="farmer"
                required
          ...
          <Button
            ...
            disabled={isPending}
          >
            {isEditing ? t('save_changes') : t('add_farm')}
          </Button>
        </form>
      </DialogContent>
    </Dialog>
